...
class Manager(ScreenManager):
    ...
    def refresh(self):
        list_add = self.client.list_add

        #add
        if len(self.client.list_add) != 0:
            for element in self.client.list_add:
                count, name = element
                self.new_element(count, name)
            self.client.list_add = []

        #sub
        if len(self.client.list_sub) == 0:
            return

        for element in self.client.list_sub:
            count, name = element
